...
function filterByWord(array, matchString) {
  let result = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].includes(matchString)) {
      result.push(array[i]);
    }
  }
  return result;
...
function getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors) {
  ...
  let shuffled = copy(allFlavors);
  let i = allFlavors.length;
  while (i--) {
